...
activate_opt([],_):- !.
activate_opt([Opt|OptsList],NoOptions) :- 
    setlog_command(Opt),
...
setlog1_str(GoalString,VarNames_or_Eqs,TimeOut,OutConstr,Res,[]) :- !,     % no options
    set_default_control,
    time_out(setlog1_str(GoalString,VarNames_or_Eqs,OutConstr,Res1),TimeOut,Res2),
    check_timeout_res(Res2,Res1,Res).
...
setlogTimeOut_opt(OptsLst,GoalString,VarNames_or_Eqs,TimeOut,Constr,Res) :- 
    ...
    (TimeOut == -1 ->
     ...
     ;
        time_out(setlog1_str(GoalString,VarNames_or_Eqs,Constr,Res1),TimeOut,Res2)
    ),
    check_timeout_res(Res2,Res1,Res).
